...
export class Calculator {
  ...
  static naiveDP(
    ...
    if (x >= 2 ** 32) {
      let bestOption: [string, Option] | null = null;
      for (const [name, option] of Object.entries(options)) {
        if (
          !bestOption ||
          option.value / option.cost > bestOption[1].value / bestOption[1].cost
        ) {
          bestOption = [name, option];
        }
      }
      const count = Math.ceil(x / bestOption![1].value);
      ...
      const cost = parseFloat((count * bestOption![1].cost).toFixed(precision));
      return {
        value: count * bestOption![1].value,
        cost: cost,
        choice: { [bestOption![0]]: count },
      };
    }
